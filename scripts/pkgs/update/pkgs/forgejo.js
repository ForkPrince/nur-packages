...
async function single(file, { config, force }) {
  ...
  let url = config.asset.file ? `${instance}/${repo}/releases/download/${prefix}${version}/${config.asset.file}` : config.asset.url
...
async function platforms(file, { config, force }) {
  ...
  if (!hasVersions) {
    ...
    for (const [platform, settings] of Object.entries(platforms)) {
      ...
      let url = settings.file ? `${instance}/${repo}/releases/download/${prefix}${version}/${settings.file}` : settings.url;
    ...
    for (const [platform, settings] of Object.entries(platforms)) {
      ...
      let url = settings.file ? `${instance}/${repo}/releases/download/${prefix}${parsed}/${settings.file}` : settings.url;
...
async function variants(file, { config, force }) {
  ...
  for (const [variant, settings] of Object.entries(variants)) {
    ...
    const url = `${instance}/${repo}/releases/download/${prefix}${version}/${name}`;
